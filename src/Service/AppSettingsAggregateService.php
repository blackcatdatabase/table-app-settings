 ...
 */
final class AppSettingsAggregateService
{
use ServiceHelpers;
    ...
    public function __construct(
        private Database $db, private AppSettingRepository $appSettingRepo
    ) {
        // Repository autowiring (generated)
        $this->repositories = [
  'app_settings' => $this->appSettingRepo
];
    }
    ...
    protected function withRowLock(callable $locker, int|string|array $id, callable $fetch, string $mode = 'wait'): mixed
    {
        ...
        return $this->txn(function () use ($locker, $id, $fetch, $mode) {
            ...
            if ($row === null) {
                ...
                throw new \BlackCat\Database\Packages\AppSettings\ModuleException("Record {$idStr} not found for locking.");
            }
