...
namespace BlackCat\Database\Packages\AppSettings\Joins;
...
final class AppSettingsJoins {
    ...
    /**
     * FK: app_settings -> users
     * LEFT JOIN vw_users AS $as ON $as.id = $alias.updated_by
     * @return array{0:string,1:array<string,mixed>}
     */
    public function joinUsers(string $alias = 't', string $as = 'j0'): array {
        [$alias, $as] = $this->assertAliasPair($alias, $as);
        return [' LEFT JOIN vw_users AS ' . $as . ' ON ' . $as . '.id = ' . $alias . '.updated_by' . ' ', []];
    }
