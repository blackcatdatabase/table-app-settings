...
namespace BlackCat\Database\Packages\AppSettings\Repository;

use BlackCat\Core\Database;
use BlackCat\Database\Packages\AppSettings\Definitions;
use BlackCat\Database\Packages\AppSettings\Criteria;
use BlackCat\Database\Packages\AppSettings\ContractRepository;

final class AppSettingRepository implements ContractRepository {
    ...
    public function insert(array $row): void {
        ...
        $sql = "INSERT INTO app_settings (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function insertMany(array $rows): void {
        ...
        $sql = "INSERT INTO app_settings (".implode(',', $cols).") VALUES ".implode(',', $allPlace);
        $this->db->execute($sql, $params);
    }
    ...
    public function upsert(array $row): void {
        ...
        $sqlBase = "INSERT INTO app_settings (".implode(',', $cols).") VALUES (".implode(',', $place).")";
    ...
    public function updateById(int|string $id, array $row): int {
        ...
        $sql = "UPDATE app_settings SET ".implode(',', $assign)." WHERE setting_key = :id".$verCond;
    ...
    public function deleteById(int|string $id): int {
        ...
        if ($soft) {
            ...
            return $this->db->execute("UPDATE app_settings SET $set WHERE setting_key = :id", $params);
        }
        return $this->db->execute("DELETE FROM app_settings WHERE setting_key = :id", [':id'=>$id]);
    }
    ...
    public function restoreById(int|string $id): int {
        ...
        $set = "$soft = NULL";
        if ($updAt && $updAt !== $soft) { $set .= ", $updAt = CURRENT_TIMESTAMP"; }
        return $this->db->execute("UPDATE app_settings SET $set WHERE setting_key = :id", [':id'=>$id]);
    }

    // ============ READ / PAGE / LOCK ============
    public function findById(int|string $id): ?array {
        $sql = "SELECT * FROM app_settings WHERE setting_key = :id";
        $row = $this->db->fetchOne($sql, [':id'=>$id]);
        return $row ?: null;
    }

    public function exists(string $whereSql = '1=1', array $params = []): bool {
        $sql = "SELECT 1 FROM app_settings WHERE $whereSql LIMIT 1";
        return (bool)$this->db->fetchOne($sql, $params);
    }

    public function count(string $whereSql = '1=1', array $params = []): int {
        $sql = "SELECT COUNT(*) FROM app_settings WHERE $whereSql";
    ...
    public function paginate(Criteria $c): array {
        ...
        $order = $order ?: (Definitions::defaultOrder() ?? 'setting_key DESC');
        $joins = $joins ?: '';

        $total = (int)$this->db->fetchOne("SELECT COUNT(*) FROM app_settings t $joins WHERE $where", $params);
        $items = $this->db->fetchAll("SELECT t.* FROM app_settings t $joins WHERE $where ORDER BY $order LIMIT $limit OFFSET $offset", $params);
    ...
    public function lockById(int|string $id): ?array {
        $sql = $this->db->isMysql()
            ? "SELECT * FROM app_settings WHERE setting_key = :id FOR UPDATE"
            : "SELECT * FROM app_settings WHERE setting_key = :id FOR UPDATE";
