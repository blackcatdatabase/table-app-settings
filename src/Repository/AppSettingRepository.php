...
final class AppSettingRepository implements RepoContract, KeysetRepoContract
{
    ...
    public function upsertMany(array $rows): int {
        ...
        if (class_exists(\BlackCat\Database\BulkUpsertRepository::class) && [ 'setting_key' ] !== []) {
            $bulk = new \BlackCat\Database\BulkUpsertRepository($this->db, \BlackCat\Database\Packages\AppSettings\Definitions::table());
            return $bulk->upsertMany($rows, [ 'setting_key' ], [ 'setting_value', 'type', 'section', 'description', 'is_protected', 'updated_at', 'updated_by', 'version' ]);
        }
