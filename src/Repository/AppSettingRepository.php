...
use BlackCat\Database\Packages\AppSettings\Dto\AppSettingDto as Dto;
use BlackCat\Database\Packages\AppSettings\Mapper\AppSettingDtoMapper as RowMapper;
...
use BlackCat\Database\Packages\AppSettings\Repository\AppSettingRepositoryInterface;
...
class AppSettingRepository implements AppSettingRepositoryInterface, RepoContract, KeysetRepoContract
{
use OrderByTools, PkTools, RepositoryHelpers;

    /** @var mixed literal token for upsert keys (array or empty). */
    private mixed $tokenUpsertKeys = [ 'setting_key' ];
     ...
     */
    protected function def(): string { return \BlackCat\Database\Packages\AppSettings\Definitions::class; }
    ...
    private function doUpsert(array $row, bool $revive): void
    {
        ...
        $updCols = [ 'setting_value', 'type', 'section', 'description', 'is_protected', 'updated_at', 'updated_by', 'version' ];
    ...
    public function upsertMany(array $rows): int {
        ...
        if ($helperKeys && class_exists(\BlackCat\Database\Support\BulkUpsertHelper::class)) {
          $bulk = new \BlackCat\Database\Support\BulkUpsertHelper($this->db, \BlackCat\Database\Packages\AppSettings\Definitions::class);
          $bulk->upsertMany($rows, $helperKeys, [ 'setting_value', 'type', 'section', 'description', 'is_protected', 'updated_at', 'updated_by', 'version' ]);
          return count($rows);
        }
