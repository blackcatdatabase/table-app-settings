...
final class Criteria extends BaseCriteria
{
    ...
    {
        $x = [ 'setting_key', 'setting_value', 'type', 'section', 'description', 'is_protected', 'updated_at', 'version', 'updated_by' ];
    ...
    public static function fromDb(
        ...
        if (\method_exists(\BlackCat\Database\Packages\AppSettings\Definitions::class, 'softDeleteColumn')) {
            $soft = \BlackCat\Database\Packages\AppSettings\Definitions::softDeleteColumn();
    ...
    public function byId(int|string $id): self {
        return $this->where('t.setting_key = :cid', ['cid' => $id]);
    }
    public function byIds(array $ids): self {
        if (!$ids) return $this->where('1=0');
        return $this->whereIn('t.setting_key', array_values($ids));
    }
